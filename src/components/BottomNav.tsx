...
export function BottomNav({ onUploadClick }: BottomNavProps) {
  ...
  const tabs = [
    ...
    { path: '/planner', icon: Sparkles, label: 'Planner', badge: overdueCount > 0 ? overdueCount : undefined },
        ...
        {tabs.slice(0, 2).map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center gap-0.5 py-2 px-3 rounded-xl transition-all duration-200 touch-feedback relative",
        ...
        {tabs.slice(2).map((tab) => {
          ...
          return (
            <button
              ...
              onClick={() => navigate(tab.path)}
              className={cn(
                "flex flex-col items-center gap-0.5 py-2 px-3 rounded-xl transition-all duration-200 touch-feedback",
                isActive ? "text-primary bg-primary/10" : "text-muted-foreground"
              )}
            >
              <Icon className="w-5 h-5" />
